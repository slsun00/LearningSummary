...
概述
    ...
    每个web.xml文件的根元素<web-app>中，都必须标明这个 web.xml使用的是哪个模式文件。
...
context-param
	...
	声明应用范围内的初始化参数, 形式为：含有一对参数名和参数值
    用于向 ServletContext提供键值对，即应用程序上下文信息
		listener, filter等在初始化时会用到这些上下文中的信息
		servlet里面可以通过getServletContext().getInitParameter("context/param")得到
    数名在整个Web应用中必须是惟一的。
...
servlet-name, 
	<!--
		定义servlet的名称，
		该名称在整个应用中必须是惟一的: servlet 类
	-->
(servlet-class|jsp-file), 
	<!--
	
	servlet-class元素：
		用来指定servlet的完全限定的名称：指定servlet的类名称
...
load-on-startup?, 
	...
	指定当Web应用启动时，装载Servlet的次序
      ...
      加载顺序
         正数或者零：从值小 -> 大的： servlet 依次加载
         空或负值: 在Web客户首次访问这个servlet时加载它
...
注意
	以”/’开头和以”/*”结尾的是用来做 路径映射的
	前缀”*.”开头的是用来做         扩展映射的
	<!--
		/*.action 匹配出错
		这个匹配即属于路径映射，也属于扩展映射，导致容器无法判断。
	-->
