...
使用
    只有删除的结点所在的路径的高度会变化，
    失衡节点的儿子
    	失衡子节点： 失衡节点.left.height  > 失衡节点.right.heifht
    失衡孙节点
    	失衡孙节点： 失衡子节点.leght.height > 失衡子节点.right.height
    
// problemNode > problemNode.left
if problemNode.left.height >= problemNode.right {
	// 决定用哪个子节点 childNode = problemNOde.left
...
// problemNode > problemNode.left    
	...
	// 决定用哪个子节点 childNode = problemNOde.right
    if (child.left.height >= child.right.height) {
        // 选用孙左节点 : child.left < child < problem
        adjustThreeNode(。。。)
    } else if (child.left.height >= child.right.height) {
